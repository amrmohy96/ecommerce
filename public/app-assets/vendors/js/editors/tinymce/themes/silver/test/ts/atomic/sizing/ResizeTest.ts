...
UnitTest.asynctest('Editor resizing tests', function (success, failure) {
    ...
    const getDimensionsTests = Log.stepsAsStep('TBA', 'Check the correct dimensions are returned', [
        ...
        makeDimensionsTest('Height less than minimum, both resize, OK width change', -500, 50, ResizeTypes.Both, 500, {
            height: 400,
            width: 550
        }),
        makeDimensionsTest('Height greater than maximum, both resize, OK width change', 500, 50, ResizeTypes.Both, 500, {
            height: 600,
            width: 550
        }),
        makeDimensionsTest('Width less than minimum, no height change', 0, -500, ResizeTypes.Both, 500, {
            height: 500,
            width: 400
        }),
        makeDimensionsTest('Width more than maximum, no height change', 0, 500, ResizeTypes.Both, 500, {
            height: 500,
            width: 600
        }),
